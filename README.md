...
We are a team full of unity and leadership, with excellent technical talents and people with leadership thinking. At the beginning, we held a meeting on data cleaning and decided not to perform overly complex operations. Instead, we will focus on sentiment classification, such as positive and negative. Apart from some input errors and word repetition, our team members suggested using the WordCloud2 package for beautification, which laid the foundation for our work. WordCloud2 can display the frequency of different words in a given text by making the size of each word proportional to its appearance, and then arrange all the words together to form a cloud pattern. It can also be arranged in any format: horizontal lines, vertical columns, or other shapes. This arrangement is highly favored by us because it not only presents data directly, but also visually appealing. At the beginning, the images we created had a lot of colors. Although they had a great visual impact, we couldn't intuitively see the differences between words and the classifications we made. Therefore, we simplified them by roughly dividing all words into two emotions and using colors to distinguish them, expressing frequency in different sizes. 
